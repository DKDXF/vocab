...
基于 **Python FastAPI + SQLite** 的本地单词学习应用，采用 **三轮记忆法** 和 **SM-2间隔重复算法** 帮助高效记忆英语单词。
...
### 🧠 科学记忆算法
- 📈 **SM-2间隔重复算法**：基于SuperMemo算法的动态复习间隔调整
- 🔄 **艾宾浩斯遗忘曲线**：1天/2天/4天/7天/15天/30天 + SM-2动态间隔
- 🎯 **强干扰项选择题**：基于形近词/义近词/混淆组生成干扰项
- ✍️ **拼写验证**：第三轮输出验证，拼写正确才算掌握
- 🏷️ **单词标签系统**：支持重难词、已掌握、很熟悉、太简单等标签分类
- 📝 **用户笔记**：可为每个单词添加个性化学习笔记
...
### 📊 数据统计与分析
- 📅 **打卡日历**：记录学习轨迹，连续打卡统计
- 📈 **多维统计**：总览统计、掌握度分布、周学习趋势
- ⏰ **智能复习计划**：按优先级排序，标注紧迫度
- 🔧 **每日新词上限**：可自定义每日新学单词数量（默认15个）
- 🌙 **熬夜模式**：支持延迟打卡时间，避免深夜学习断签
- 📊 **三轮进度统计**：各阶段单词数量一目了然
- 📋 **记忆历史追踪**：记录每次复习结果，计算记忆率

### 🌟 高级复习功能
- 🕒 **昨日重现**：针对过去4天内遗忘的单词进行强化复习
- 🏷️ **单词标签**：支持标记重难词、已掌握、很熟悉、太简单等状态
- 📝 **个人笔记**：为每个单词添加个性化学习笔记
- 🔄 **多模式复习**：选择释义、拼写验证、听音辨意等多种复习方式

## 🏗️ 项目结构

```
vocab-master/
├── main.py              # FastAPI 应用入口，包含认证中间件
...
│   ├── wordreview.py    # 高级复习 API（昨日重现、标签、笔记）
...
├── 词书导入完整模板.csv  # 完整的词书导入模板
├── 词书导入模板.csv     # 简化的词书导入模板
└── README.md            # 本文件
```
...
### 第二轮：间隔复习
基于艾宾浩斯遗忘曲线自动安排复习时间（1天→2天→4天→7天→15天→30天→SM-2动态），自评掌握程度。
...
VocabMaster实现了经典的SM-2间隔重复算法，根据用户对单词的掌握程度动态调整复习间隔：
...
| requests | HTTP 请求库（爬虫） |
| beautifulsoup4 | HTML 解析库（爬虫） |
| lxml | XML/HTML 解析器（爬虫） |
